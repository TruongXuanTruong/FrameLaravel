                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        @endphp
                            ...
                            <td scope="row">{{$t}}</td>
                            <td>
                                <img src="{{ asset($row->options->thumbnail) }}" width="100" alt="{{ $row->name }}">
                            </td>
                            <td scope="col"><a href="">{{$row->name}}</a></td>
                            <td scope="col">{{number_format($row->price,0,',','.')}}đ</td>
                            <td scope="col">
                                <input type="hidden" name="rowId[]" value="{{ $row->rowId }}">
                                <input type="number" name="qty[{{$row->rowId}}]" min="1" style="width:50px; text-align:center" value="{{$row->qty}}">
                            </td>
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <td><strong>{{Cart::total()}}</strong>đ</td>
                        </tr>
                    </tfoot>
                </table>
